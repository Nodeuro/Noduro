...
By default, propiwtary points are tracked as opposed to all of them, with selected values being chosen to maximize runtime viability whilst maninting accuracy.
...
def convert_stream(image):
    print(base64.b64encode(cv2.imencode('.jpg', image)[1].tobytes()).decode('ascii'))
    x = 0
class gesture_tracker(gesture_base):
    ...
    def looping_analysis(self, videoCapture : object, video_shape = None, fps = None,  result_vid : str = None, starting_vid: str = None, frame_skip : int = None, save_pose : bool = False, standardize_pose : bool = False, save_frames = False):
        ...
        while True:
            frame_number +=1
            _, frame = videoCapture.read()
            self.track[frame_number]["read"] = time.time() - self.track[frame_number]["start"];self.track[frame_number]["start"] = time.time()
            ...
            self.track[frame_number]["boolean"] = time.time() - self.track[frame_number]["start"];self.track[frame_number]["start"] = time.time()
            
            if frame_skip != 0 and frame_number % frame_skip == 0 : #if you want to skip frames
                frame = self.per_frame_analysis(frame, True) #run frame analysis
                _ = self.while_processing(frame,True) #filler for any subclasses
                self.track[frame_number]["perframe"] = time.time() - self.track[frame_number]["start"];self.track[frame_number]["start"] = time.time()
                if _ is not None:
                    frame = _
                ...
                if standardize_pose:
                    try:
                        ...
                        pass
                self.track[frame_number]["standardize_pose"] = time.time() - self.track[frame_number]["start"];self.track[frame_number]["start"] = time.time()
            else:
            ...
            self.track[frame_number]["displays"] = time.time() - self.track[frame_number]["start"];self.track[frame_number]["start"] = time.time()
            ...
            self.track[frame_number]["convert_stream"] = time.time() - self.track[frame_number]["start"];self.track[frame_number]["start"] = time.time()
            ...
            if cv2.waitKey(1) == ord('q'): #stop everything
                ...
                cv2.destroyAllWindows()
                break
            self.track[frame_number]["last"] = time.time()
...
if __name__ == '__main__':
    ...
    a.realtime_analysis(frame_skip=10)
